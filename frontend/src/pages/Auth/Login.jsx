...
export default function Login() {
  ...
  async function onSubmit(e) {
    ...
    try {
      ...
      if (resp?.needsVerification) {
        setUnverifiedEmail(resp.email || email);
  ...
  async function handleResend() {
    ...
    setError('');
    try {
      await apiClient.post('/api/auth/resend-verification', { email: unverifiedEmail });
      ...
      <div className="w-full max-w-md rounded-2xl border border-white/10 bg-slate-950/60 p-6">
        ...
        {unverifiedEmail ? (
          <div className="mt-4 space-y-3">
            <div className="rounded-lg border border-yellow-500/20 bg-yellow-500/10 px-4 py-3 text-xs text-yellow-200">
